...
export R1_AOA_CONFIG='{
      "aoa_statements": [
        "ernie.layers.0.mlp.up_gate_proj.weight -> _",
      ]
    }'
...
run_with_yaml "$case_temp1_out_dir" "$case_temp1_log_dir" "${case_temp0_out_dir}/checkpoint-5" 6 6 \
    ...
    "${R1_VIRTUAL_PP_DEGREE}" "${R1_AOA_CONFIG}"
...
########################################
# 校验 1：比较 Round 2 与 Round 0 的 ckpt MD5 是否一致（除被移除的目标参数张量外，其余键的 MD5 应完全一致）
########################################
...
## 说明：仅被移除的目标参数张量允许 MD5 不一致；其余键应一致