...
# # 清理旧目录
rm -rf "$case_temp3_out_dir" "$case_temp3_log_dir" \
...
trainer_args:
    ...
    aoa_config: {
        "aoa_statements": [
        "ernie.layers.0.input_layernorm.weight -> ernie.layers.0.input_layernorm_rename.weight"
        ]
    }
EOF
...
run_with_yaml "$case_temp3_out_dir" "$case_temp3_log_dir" "${case_temp0_out_dir}/checkpoint-5" 7 7 \
...
# 1. round0测试时不配置aoa
# 2. round3测试时配置aoa
# 3. 需要在ernie_moe组网中修改self.input_layernorm为self.input_layernorm_rename