...
export R2_AOA_CONFIG='{
      "aoa_statements": [
        "_ -> ernie.layers.0.post_attention_layernorm_test.weight",
      ]
    }'
...
run_with_yaml() {
    ...
    local AOA_CONFIG_IN="${13}"  # aoa_config（默认为空）
...
#     "${R0_VIRTUAL_PP_DEGREE}" "${R0_AOA_CONFIG}"
...
########################################
export CUDA_VISIBLE_DEVICES=0,1
run_with_yaml "$case_temp2_out_dir" "$case_temp2_log_dir" "${case_temp0_out_dir}/checkpoint-5" 6 6 \
    ...
    "${R2_VIRTUAL_PP_DEGREE}" "${R2_AOA_CONFIG}"



########################################
# 校验 1：比较 Round 2 与 Round 0 的 ckpt md5 是否一致（只有remove的那个key对应的tensor的md5不一致）
########################################
if [ -f "/home/paddlenlp/llm/compare_checkpoints.py" ]; then
    python /home/paddlenlp/llm/compare_checkpoints.py "${case_temp2_out_dir}/checkpoint-6" "${case_temp0_out_dir}/checkpoint-5"
else
...
# 注意: 需要在ernie_moe组网中添加一层，比如post_attention_layernorm_test
# 比较md5，仅仅只有添加的这层key是多出来的