...
export R2_SHARDING_PARALLEL_DEGREE=2
export R2_VIRTUAL_PP_DEGREE=1
export R2_SHARD_CFG="split_param"


export R3_MOE_GROUP="tp"
export R3_DATA_PARALLEL_DEGREE=1
export R3_TENSOR_PARALLEL_DEGREE=2
export R3_PIPELINE_PARALLEL_DEGREE=2
export R3_EXPERT_PARALLEL_DEGREE=2
export R3_SHARDING_PARALLEL_DEGREE=1
export R3_VIRTUAL_PP_DEGREE=1
export R3_SHARD_CFG="split_param"


# 统一根目录与任务名（对齐 run_pretrain_llm.sh 的结构）
ROOT_DIR="/home/ERNIE/examples/pre-training"
task_name="SD2_TP2(EP2)_to_PP2_TP2(EP2)"
...
run_with_yaml() {
    ...
    local SHARD_CFG_IN="${13}" # sharding_parallel_config（未传入则默认空字符串）
...
trainer_args:
    ...
    sharding_parallel_config: "${SHARD_CFG_IN}"
    sequence_parallel: 1
    amp_master_grad: 1
...
run_with_yaml "$case_temp0_out_dir" "$case_temp0_log_dir" "" 7 5 \
    ...
    "${R0_VIRTUAL_PP_DEGREE}" "${R0_SHARD_CFG}"
...
run_with_yaml "$case_temp1_out_dir" "$case_temp1_log_dir" "${case_temp0_out_dir}/checkpoint-5" 6 6 \
    ...
    "${R1_VIRTUAL_PP_DEGREE}" "${R1_SHARD_CFG}"
...
run_with_yaml "$case_temp2_out_dir" "$case_temp2_log_dir" "${case_temp1_out_dir}/checkpoint-6" 7 7 \
    ...
    "${R2_VIRTUAL_PP_DEGREE}" "${R2_SHARD_CFG}"
...
run_with_yaml "$case_temp3_out_dir" "$case_temp3_log_dir" "${case_temp0_out_dir}/checkpoint-5" 10 11 \
    ...
    "${R3_VIRTUAL_PP_DEGREE}" "${R3_SHARD_CFG}"
