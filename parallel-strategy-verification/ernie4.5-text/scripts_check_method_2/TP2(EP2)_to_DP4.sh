...
export R0_MOE_GROUP="tp"
export R0_DATA_PARALLEL_DEGREE=1
export R0_TENSOR_PARALLEL_DEGREE=2
...
export R1_TENSOR_PARALLEL_DEGREE=2
...
export R2_DATA_PARALLEL_DEGREE=4
...
if [ -d "$SRC_CKPT_6_A" ]; then
    echo "CONFIG_FLAG_A: $CONFIG_FLAG_A"
...
if [ -d "$SRC_CKPT_6_B" ]; then
    echo "CONFIG_FLAG_B: $CONFIG_FLAG_B"
    python /home/ERNIE/examples/pre-training/transform_from_distcp_to_safetensors.py \
        "$SRC_CKPT_6_B" \
        "$MERGE_OUT_6" \
        "$CONFIG_FLAG_B"
...
COMPARE_LOG="${ROOT_DIR}/compare_md5_${task_name}.log"
rm -f "$COMPARE_LOG"
echo "[COMPARE LOG] $(date)" | tee -a "$COMPARE_LOG"

if [ -d "$MERGE_OUT_5" ] && [ -d "$BASELINE_CKPT_5" ]; then
    echo "\n===== Compare MERGE_OUT_5 vs BASELINE_CKPT_5 =====" | tee -a "$COMPARE_LOG"
    echo "MERGE_OUT_5=$MERGE_OUT_5" | tee -a "$COMPARE_LOG"
    echo "BASELINE_CKPT_5=$BASELINE_CKPT_5" | tee -a "$COMPARE_LOG"
    python /home/ERNIE/examples/pre-training/compare_safetensors_md5.py "$MERGE_OUT_5" "$BASELINE_CKPT_5" | tee -a "$COMPARE_LOG"
else
    echo "比较所需目录不存在：$MERGE_OUT_5 或 $BASELINE_CKPT_5"
fi

if [ -d "$MERGE_OUT_6" ] && [ -d "$BASELINE_CKPT_5" ]; then
    echo "\n===== Compare MERGE_OUT_6 vs BASELINE_CKPT_5 =====" | tee -a "$COMPARE_LOG"
    echo "MERGE_OUT_6=$MERGE_OUT_6" | tee -a "$COMPARE_LOG"
    echo "BASELINE_CKPT_5=$BASELINE_CKPT_5" | tee -a "$COMPARE_LOG"
    python /home/ERNIE/examples/pre-training/compare_safetensors_md5.py "$MERGE_OUT_6" "$BASELINE_CKPT_5" | tee -a "$COMPARE_LOG"
else
