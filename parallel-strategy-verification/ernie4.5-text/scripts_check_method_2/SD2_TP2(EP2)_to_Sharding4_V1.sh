...
run_with_yaml() {
    ...
    local SHARD_CFG_IN="${15}"      # sharding_parallel_config（未传入则默认 split_param）
    ...
    local SHARD_CFG_VAL=${SHARD_CFG_IN:-""}
...
run_with_yaml "$case_temp0_out_dir" "$case_temp0_log_dir" "" 6 5 \
    ...
    "${R0_VIRTUAL_PP_DEGREE}" "${R0_UNIFIED_CKPT}" "${R0_FLEX_CKPT}" "${R0_SHARD_CFG}"
...
run_with_yaml "$case_temp1_out_dir" "$case_temp1_log_dir" "" 6 5 \
    ...
    "${R1_VIRTUAL_PP_DEGREE}" "${R1_UNIFIED_CKPT}" "${R1_FLEX_CKPT}" "${R1_SHARD_CFG}"
...
run_with_yaml "$case_temp2_out_dir" "$case_temp2_log_dir" "${case_temp1_out_dir}/checkpoint-5" 6 6 \
    ...
    "${R2_VIRTUAL_PP_DEGREE}" "${R2_UNIFIED_CKPT}" "${R2_FLEX_CKPT}" "${R2_SHARD_CFG}"
